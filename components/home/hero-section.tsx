...
type AuthLink =
  | AuthLinkTypeLink
  | AuthLinkTypeButtonOutline
  | AuthLinkTypeButtonPrimary;
...
const AbstractBackground: React.FC = () => {
  return (
    <div>
      <div className="absolute inset-0 bg-gradient-to-br from-transparent via-transparent to-blue-700 opacity-50 z-0"></div>{" "}
...
const Navbar: React.FC<NavbarProps> = ({
  ...
  const renderLogo = () => (
    <div className="flex items-center space-x-2">
      ...
      >
        ...
        <path
          d="M6 12H10V16H6V12ZM14 8H18V16H14V8ZM10 4H14V8H10V4Z"
          fill="white"
        />
      </svg>
        ...
        if (link.type === "link") {
          return (
            <a
              key={index}
              href={link.href}
              className={`text-gray-600 hover:text-orange-600 ${link.className}`}
            >
              {link.text}
            </a>
          );
  ...
  return (
    <div className="relative z-10 max-w-[1440px] w-full px-4 sm:px-6 lg:px-8 mx-auto mb-4">
      <nav className="bg-white/70 backdrop-blur-md border border-white/20 flex items-center justify-between py-4 px-8 lg:px-12 lg:py-4 shadow-sm">
        {renderLogo()}
    ...
    <div className="flex flex-col space-y-4 mt-8">
      {links.map((link, index) => (
        <a
          key={index}
          href={link.href}
          className="text-gray-800 text-lg font-medium hover:text-orange-600"
          onClick={onClose}
        >
          {link.text}
        </a>
        ...
        if (link.type === "link") {
          return (
            <a
              key={index}
              href={link.href}
              className={`text-gray-600 hover:text-orange-600 text-lg ${link.className}`}
              onClick={onClose}
            >
              {link.text}
            </a>
        ...
        <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-8">
          {secondaryCtaText && (
            <a
              ...
              className="w-full sm:w-auto px-6 py-3 border border-gray-300 text-gray-800 font-semibold hover:bg-gray-50 text-sm"
            >
              {secondaryCtaText}
            </a>
          )}
          {primaryCtaText && (
            <a
              href={primaryCtaLink}
              className="w-full sm:w-auto px-6 py-3 bg-orange-500 text-white font-semibold hover:bg-orange-600 text-sm"
            >
              {primaryCtaText}
            </a>
          )}
...
const AppPreviewHeaderControls: React.FC<AppPreviewHeaderControlsProps> = ({
  filters,
  actions,
}) => {
  ...
  const renderFilterButton = (filter: AppPreviewFilter, index: number) => (
    <button
      key={index}
      className="flex items-center px-2 py-1 border border-gray-300 text-xs hover:bg-gray-100"
    >
  ...
  const renderActionButton = (action: AppPreviewAction, index: number) => {
    let classes = "px-2 py-1  text-xs font-semibold";
...
const AppPreviewHeader: React.FC<AppPreviewHeaderProps> = ({
  headerControls,
}) => {
  ...
  return (
    <div className="bg-white -md shadow-inner overflow-hidden border border-gray-200">
      ...
      {data.map((row, index) => (
        <div
          key={row.id || index}
          className={`grid grid-cols-[1.8fr_1.2fr_0.8fr_1fr_1.2fr] gap-4 text-sm text-gray-700 px-4 py-2 items-center cursor-pointer hover:bg-gray-50 ${index < data.length - 1 ? "border-b border-gray-200" : ""}`}
        >
          ...
          <div className="text-gray-700 truncate">{row.project}</div>
        </div>
      ))}
    </div>
  );
