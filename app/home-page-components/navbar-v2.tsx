...
export default function NavbarV2() {
    ...
    return (
        <motion.nav
            ...
            className="w-full py-4 sticky top-0 z-50 border-b border-black/5 backdrop-blur-md"
        >
            <div className="mx-auto w-full max-w-[1400px] px-4 sm:px-6 lg:px-8 flex items-center justify-between">
                {/* Logo */}
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    <motion.div
                        ...
                        className="absolute top-full left-0 right-0 mt-2 bg-[#0B0D14] border border-white/10 rounded-2xl p-4 md:hidden flex flex-col gap-2 shadow-2xl overflow-hidden mx-4"
                    >
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
